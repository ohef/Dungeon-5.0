...
class basic_store {
 ...
 private:
  static basic_store<state_t> create(const std::function<state_t(state_t, action)> reducerr, const state_t initial_state,
                                     const TOptional<action> initial_action) {
    auto state = (initial_action) ? reducerr(initial_state, *initial_action) : initial_state;
    return basic_store<state_t>(reducerr, state);
  }

  basic_store(reducer_t<state_t> reducerr, state_t initial_state) : _reducer(reducerr), _current_state(initial_state) {
    _dispatcher = [&](action_t action) -> action_t {
      ...
      Algo::ForEach(_subscribers, [&](const auto& pair) { pair.second(_current_state); });
  ...
  basic_store(reducer_t<state_t> reducerr, state_t initial_state,
              std::initializer_list<std::function<dispatch_transformer_t(basic_middleware<state_t>)>> transformer)
      : basic_store(reducerr, initial_state) {
