...
{
	TSharedPtr<FDataTableRowDragDropOp> DataTableRowDragDropOp = DragDropEvent.GetOperationAs<FDataTableRowDragDropOp>();
		...
		for (int y = 0; y < height; y++)
		{
			...
			[
				...
				.OnClicked_Lambda([x,y,this]
				{
					 ...
					 *StructOnScope->Get() = lager::view(unitDataLens(possibleUnit.Get(-1)),
														 editorStore->get()).Get({});
			...
			[SNew(SButton)
				...
				.OnClicked_Lambda([this]
				{
					editorStore->dispatch(CreateDungeonAction(FSpawnUnit{
						.position = contextPosition, .unit = *StructOnScope->Get()
					}));
					return FReply::Handled();
				})]
		]
